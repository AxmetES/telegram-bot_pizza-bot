...
def create_customer(customer_data, shop_token):
...
def creat_product(url, shop_token, pizza):
    headers = {
        'Authorization': f'Bearer {shop_token}',
        'Content-Type': 'application/json',
    }
    data = {"data": {"type": "product", "name": pizza['name'], "slug": str(pizza['id']) + '-pizza',
                     "sku": str(pizza['id']),
...
def create_file(url, shop_token, image_url):
...
def get_cart(chat_id, shop_token):
...
def add_product_to_cart(chat_id, shop_token, payload_cart):
...
def get_cart_total(shop_token, chat_id):
...
def get_products(product_url, shop_token):
    headers = {
        'Authorization': f'Bearer {shop_token}'
    }
...
def get_product(products_id, shop_token):
...
def get_file_url(main_image_id, shop_token):
    headers = {
        f"Authorization": shop_token,
    }
...
def load_menu_to_pizzeria(menu, shop_token):
    for pizza in menu:
        try:
            created_product_id = creat_product(products_url, shop_token, pizza)['data']['id']
        ...
        image_file_id = \
            create_file(file_url, shop_token, image_url=pizza['product_image']['url'])[
                'data']['id']
...
def delete_cart_product(chat_id, product_id, shop_token):
...
def get_all_products(products_url, shop_token):
    headers = {
        'Authorization': f'Bearer {shop_token}',
    }
...
def get_all_files(file_url, shop_token):
    headers = {
        'Authorization': f'Bearer {shop_token}',
    }
    response = requests.get(file_url, headers=headers)
...
def delete_products(shop_token, product_id):
...
def delete_files(shop_token, file_id):
...
if __name__ == '__main__':
    ...
    shop_token = get_shop_token(token_url)['access_token']
    print(f' shop token ---- {shop_token}')

    load_menu_to_pizzeria(menu, shop_token)
    # all_files = get_all_products(file_url, shop_token)
    # for file in all_files['data']:
    #     delete_files(shop_token, file['id'])
    #
    # all_products = get_all_products(products_url, shop_token)
    # for products in all_products['data']:
    #     delete_products(shop_token, products['id'])
